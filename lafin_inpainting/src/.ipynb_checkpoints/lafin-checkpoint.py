    ...
    def __init__(self, config):
        ...
        if self.config.MODE == 1:
            if self.config.MODEL == 1 :
                self.train_dataset = Dataset(config,config.TRAIN_FLIST, config.TRAIN_LANDMARK_DIR, config.TRAIN_MASK_DIR, augment=True,training=True)
                self.val_dataset = Dataset(config,config.VAL_FLIST,config.VAL_LANDMARK_DIR, config.VAL_MASK_DIR, augment=True, training=True)
                self.sample_iterator = self.val_dataset.create_iterator(config.SAMPLE_SIZE)

            elif self.config.MODEL == 2:

                self.train_dataset = Dataset(config, config.TRAIN_FLIST, config.TRAIN_LANDMARK_DIR,
                                             config.TRAIN_MASK_DIR, augment=True, training=True)
                self.val_dataset = Dataset(config, config.VAL_FLIST, config.VAL_LANDMARK_DIR,
                                           config.VAL_MASK_DIR, augment=True, training=True)
                self.sample_iterator = self.val_dataset.create_iterator(config.SAMPLE_SIZE)
        ...
        while(keep_training):
            ...
            for index, items in enumerate(train_loader):
                ...
                # inpaint model
                    ...
                    if index % 200 == 0:
                        wandb.log({"epoch": epoch, "batch_idx": index, "gan_loss": gen_loss.item(), "dis_loss":dis_loss.item()})
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint model
                ...
                #print (landmark_mask_image.shape)
                #print (landmarks)
